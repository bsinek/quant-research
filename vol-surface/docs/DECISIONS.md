...
## 013. Global SSVI: arbitrage-free by construction, at a fit-accuracy cost — 2026-07-20
- **Decision:** `engine/ssvi.py` fits one global SSVI `(ρ, η, γ)` (power-law `φ(θ)=η θ^{-γ}`) across all quotes at once, with `θ_T` (ATM total variance per expiry) read off the raw-SVI ATM and forced non-decreasing in T. `fit_ssvi` minimizes weighted squared error under the Gatheral-Jacquier Thm 4.2 butterfly constraints (SLSQP), so the surface is butterfly-free *by construction*, not merely checked. Kept as a separate module from `svi.py`; results shown in a new `notebooks/surface_svi.ipynb` (self-contained, not surgically added to the 1.5 MB `surface.ipynb`).
- **Why:** SSVI's whole value is *explicit* no-arb: enforce the θφ inequalities and the surface can't be arbitraged on either axis, unlike independent raw-SVI slices which can cross. On the 07-13 snapshot the constrained global fit is butterfly-free and calendar-monotone. θ_T comes from the raw-SVI slices (`svi_w(0, ·)`), so the raw fit feeds SSVI rather than being throwaway.
- **Trade-off:** A single global `(ρ, η, γ)` is rigid across a 1–1985 d range: median SSVI fit error ≈ 1 vol pt (≈ 2 vol pts at the short end) vs the near-exact per-slice SVI — the standard "guaranteed no-arb vs accuracy" tension. eSSVI (θ-dependent ρ) would tighten the short end and is the natural next step. On this snapshot raw θ_T was already monotone, so the calendar diagnostic that motivates SSVI didn't fire at ATM — the motivation here is the *guarantee*, not an observed ATM crossing.

## 012. Raw SVI per expiry: Zeliade quasi-explicit fit, vega/spread weighting — 2026-07-20
- **Decision:** `engine/svi.py` fits raw SVI per expiry in total-variance space. `fit_slice` uses the Zeliade quasi-explicit method: with `(m, σ)` fixed the model is linear in `(a, bρ, b)`, so an exact weighted least-squares solve sits inside a 2-D Nelder-Mead search over `(m, σ)`. Quotes are weighted by `vega/spread` (first power, i.e. inverse-*std*, not the inverse-variance square). Butterfly is verified after the fit with `svi_g` (`g(k) ≥ 0`, Gatheral-Jacquier Lemma 2.2), not constrained during it.
- **Why:** Handing all 5 params to an optimizer lands in local minima on a fraction of the 55 expiries; the quasi-explicit split makes 3 of 5 params an exact O(1) solve and shrinks the search to a 2-D basin that's reliably found from data-driven starts. Weighting in vol space needs the price→vol conversion, which *is* vega (`vol uncertainty ≈ spread/vega`); the first-power (not squared) choice is a deliberate relaxation from the statistical optimum to keep the skew-bearing wings in the fit. Validated by synthetic round-trip (clean recovery ~4e-7) and, for `svi_g`, sign-agreement with the Breeden-Litzenberger density (100%). On 07-13: 55/55 expiries fit, median w-space RMSE ~1e-4, ~98% butterfly-free.
- **Trade-off:** Unconstrained least-squares means a slice *can* be butterfly-violating (≈2% here, wing-driven) — caught by the post-fit `g(k)` check, not prevented. First-power weighting trades a little ATM precision for wing fidelity. Per-slice independence gives no calendar guarantee (that's SSVI's job, ADR 013).
