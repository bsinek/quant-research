...
- `docs/STATE.md` — date-stamped snapshot (working / in flight / next / deferred), **one terse line per item: name + status, never its API**. Update at every phase-boundary commit, in the same commit — update ARCHITECTURE if structure changed, append DECISIONS if a decision-with-a-tradeoff was made. Never promote a deferred idea into Next without Ben.
